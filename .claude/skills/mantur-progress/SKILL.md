...
- **חיפוש "לכל מקום" (Everywhere)** (`app/flights/anywhere/`, `searchAnywhere` ב-data.ts): בשדה "לאן" אפשרות נעוצה "🌍 כל היעדים" (`ANYWHERE` ב-`LocationInput`), **עם או בלי תאריך**. עם תאריך → `prices_for_dates` עם `origin` בלבד; בלי תאריך → `v1/city-directions`. דדופ ליעד + מיון מהזול, סינון שבת חל. רשת כרטיסי-יעד (שם עברי מ-`places-he` + מפת מטרו, תמונה מ-`getDestinationImage`) → לחיצה מובילה ל-`/flights` של אותו יעד בתאריכים הזולים שנמצאו. אומת חי: 24 יעדים ממוינים, marker בהפניות.
